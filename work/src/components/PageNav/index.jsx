...
class Com extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			startPage:
				parseInt(defaultCurrent) - 2 > 0
					? parseInt(defaultCurrent) - 2
					: parseInt(defaultCurrent), //分组开始页码
			totalPage: total, //总页数
		};
	}
	createPage(pageCfg) {
	...
	render() {
		...
		const localCurrent =
			parseInt(sessionStorage.getItem(`currentPage${this.state.url}`)) || 1;
		const pageCfg = {
